...
async function askOptions() {
  return await import_prompts.default([
    {
      type: "confirm",
      name: "prettier",
...
async function newProject(name) {
  ...
  await runCommand(["elysia", "new", name], path);
  const options = await askOptions();
  for (const folder of folders) {
    await mkdir(join(path, "src", folder), { recursive: true });
  }
  if (options.prettier) {
    await writeFile(join(path, ".prettierrc"), JSON.stringify({
      semi: true,
      singleQuote: true,
      trailingComma: "all"
    }, null, 2));
    await runCommand(["bun", "add", "-D", "prettier"], path);
  }
  if (options.eslint) {
    await runCommand(["bun", "add", "-D", "eslint"], path);
    await runCommand(["bunx", "eslint", "--init"], path);
  }
  if (options.git) {
    await runCommand(["git", "init"], path);
    await runCommand(["git", "add", "."], path);
    await runCommand(["git", "commit", "-m", `feat: setup project ${name}`], path);
  }
  if (options.husky) {
    await runCommand(["bun", "add", "-D", "husky", "@commitlint/cli", "@commitlint/config-conventional"], path);
    await runCommand(["bunx", "husky", "install"], path);
    await writeFile(join(path, "commitlint.config.js"), 'module.exports = { extends: ["@commitlint/config-conventional"] };');
    await runCommand(["bunx", "husky", "add", ".husky/commit-msg", "bunx commitlint --edit $1"], path);
  }
